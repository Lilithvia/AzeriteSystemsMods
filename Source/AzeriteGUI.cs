...
namespace AzeriteSystems.ASGUI
{
	...
	public class ASGUI : MonoBehaviour
	{
		...
		public string dllLoc { get; } = AssemblyLoader.GetPathByType(typeof(ASGUI)).Replace("/PluginData/AzeriteSystems", Path.DirectorySeparatorChar.ToString());
		bool showChangelog;
		...
		{
			LoadChangelog();
			Debug.Log("[AzSy] dllLoc = " + dllLoc);
		}
		private void OnGUI()
		{
			if (showChangelog)
			{
				changelogRect = GUILayout.Window(0, changelogRect, DrawChangelogWindow, "Azerite Systems Changelog", GUILayout.Width(300), GUILayout.Height(400));
			}
		}
		...
		{
			Debug.Log("[AzSy] Loading changelog...");
			ConfigNode versionCfg = ConfigNode.Load(Path.Combine(dllLoc, "Changelog.cfg"));
			if(!versionCfg.HasValue("showChangelog"))
			{
				Debug.Log("[AzSy] Unable to find value showChangelog in Changelog.cfg");
				return;
			}
			if(!versionCfg.TryGetValue("showChangelog", ref showChangelog))
			{
				Debug.Log("[AzSy] Unable to load value from 'showChangelog'.");
				return;
			}
			if(!versionCfg.SetValue("showChangelog", false))
			{
				Debug.Log("[AzSy] Unable to set value 'showChangelog'.");
				return;
			}
			versionCfg.Save(Path.Combine(dllLoc, "Changelog.cfg"));
			if(!showChangelog)
			{
				Debug.Log("[AzSy] Changelog load aborted due to showChangelog being set to false.");
			}
